...
const SingleSeries = ({ singleSeries }) => {
  const { name, first_air_date, vote_average } = singleSeries;
  const navigate = useNavigate();

  const handleClick = () => {
    navigate(`/series/${singleSeries.id}`);
  };

  // Define the poster URL, fallback to placeholder if not available
  const posterUrl = singleSeries.poster_path 
    ? `https://image.tmdb.org/t/p/w500${singleSeries.poster_path}`
  ...
  return (
    <div className="bg-gray-800 p-2 shadow-lg max-w-full sm:max-w-72 mx-2 sm:mx-4 cursor-pointer transition-transform duration-800 ease-in transform hover:scale-110 hover:shadow-red-800 mb-8" onClick={handleClick}>
      ...
      />
      <h3 className="text-sm font-semibold pt-2 pb-2 pl-2 text-white font-poppins truncate">{name}</h3>
      <div className="flex items-center justify-between pt-1 pb-4 pl-2 pr-2">
        <p className="text-gray-400 text-xs sm:text-sm truncate">{first_air_date}</p>
